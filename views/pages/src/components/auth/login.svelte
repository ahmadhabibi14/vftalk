<script lang="ts" type="module">
  ...
  async function Login() {
    if (username == '' || password == '') return growl.showWarning('Please fill all fields');
    isSubmitted = true;
    await axios({
			method: 'post',
			url: '/api/login',
...
<div class="w-[500px] h-fit bg-white shadow-md p-5 rounded-md flex flex-col gap-4">
  ...
  <span class="text-sm text-center font-semibold">Have no account? <a href="/register" class="text-emerald-700 hover:underline">Register</a></span>
</div>