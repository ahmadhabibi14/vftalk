...
type Dockermailserver struct {
	configs.DockermailserverConf
	client *mail.Client
}

func NewDockermailserver() (*Dockermailserver, error) {
	res := &Dockermailserver{
		DockermailserverConf: configs.EnvDockermailserver(),
	}
	err := res.Connect()
	return res, err
}
...
func (m *Dockermailserver) Connect() error {
	if m.client != nil {
		err := m.client.Close()
		if err != nil {
			log.Println("dockermailserver connection close")
		}
	}
	var err error
...
func (m *Dockermailserver) SendEmail(toEmailName map[string]string, subject, text, html string) error {
	msg := mail.NewMsg()
